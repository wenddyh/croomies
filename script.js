document.addEventListener('DOMContentLoaded', function() {
    const chores = [
        "Lunes Almuerzo", "Lunes Cena", "Lunes ver si hay que comprar cosas", "Lunes Piedras Robin", "Lunes Basura",
        "Lunes Platos Almuerzo", "Lunes Platos Cena", "Lunes Ordenar Living", "Lunes y Martes Chia Puddings",
        "Martes Almuerzo", "Martes Cena", "Martes ver si hay que comprar cosas", "Martes Piedras Robin", "Martes Basura",
        "Martes Platos Almuerzo", "Martes Platos Cena", "Martes Ordenar Living",
        "Miercoles Almuerzo", "Miercoles Cena", "Miercoles ver si hay que comprar cosas", "Miercoles Piedras Robin", "Miercoles Basura",
        "Miercoles Platos Almuerzo", "Miercoles Platos Cena", "Miercoles Ordenar Living", "Miercoles y Jueves Chia Puddings",
        "Jueves Almuerzo", "Jueves Cena", "Jueves ver si hay que comprar cosas", "Jueves Piedras Robin", "Jueves Basura",
        "Jueves Platos Almuerzo", "Jueves Platos Cena", "Jueves Ordenar Living",
        "Viernes Almuerzo", "Viernes Cena", "Viernes ver si hay que comprar cosas", "Viernes Piedras Robin", "Viernes Basura",
        "Viernes Platos Almuerzo", "Viernes Platos Cena", "Viernes Ordenar Living", "Viernes y Sabado Chia Puddings",
        "Limpiar baño"
    ];
    ...
    function assignChores(chores) {
        ...
        shuffledChores.forEach((chore, index) => {
            if (chores.length % 2 === 0) {
                if (index % 2 === 0) { wendyChores.push(chore); } else { damianChores.push(chore); }
            } else {
                if (index % 2 === 0) { damianChores.push(chore); } else { wendyChores.push(chore); }
            }
        });

        if (chores.length % 2 !== 0) { damianChores.sort(); wendyChores.sort(); }
    ...
    function displayChores(assignedChores) {
        ...
        sortedWendyChores.forEach(chore => createChoreItem(chore, wendyChoresList));
        sortedDamianChores.forEach(chore => createChoreItem(chore, damianChoresList));

        updateProgressBars(assignedChores);
    }

    function sortChoresByDay(chores) {
        const dayOrder = { "Lunes": 1, "Martes": 2, "Miercoles": 3, "Jueves": 4, "Viernes": 5 };
        return chores.sort((a, b) => (dayOrder[a.split(" ")[0]] || 6) - (dayOrder[b.split(" ")[0]] || 6));
    }
    ...
    function createChoreItem(chore, list) {
        ...
        if (completedChores.includes(chore)) { listItem.classList.add('completed'); }
    ...
    function updateProgressBars(assignedChores) {
        const wendyCompleted = assignedChores.wendyChores.filter(chore => JSON.parse(localStorage.getItem('completedChores'))?.includes(chore)).length;
        const damianCompleted = assignedChores.damianChores.filter(chore => JSON.parse(localStorage.getItem('completedChores'))?.includes(chore)).length;
    ...
    sortButton.addEventListener('click', function() {
        ...
        if (completedChores.length > 0) {
            ...
            assignedChores = assignChores(chores);
        } else {
            assignedChores = assignChores(chores);
        }
        ...
        // Send data to Make
        fetch('https://hook.us2.make.com/wbd5rg7e1mhvseoy83v4oikn6bmuhmli', { // Replace with your webhook URL
            method: 'POST',
    ...
    // --- UI Logic ---
     ...
     function alternateBackgroundColors() {
            const bgColors = [['#FF5733', '#33A8FF'], ['#FFC300', '#8A2BE2'], ['#2ECC71', '#E74C3C'], ['#9B59B6', '#F39C12']];
            const titleColors = ['#FFD700', '#00FFFF', '#FF69B4', '#ADFF2F'];
            const boxColors = [['#483D8B', '#2F4F4F'], ['#800000', '#006400'], ['#A0522D', '#4682B4'], ['#556B2F', '#8B008B']];
            const buttonColors = ['#800080', '#008080', '#FFA500', '#00FF00'];
